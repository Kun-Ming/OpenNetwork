function simulation(obj, time_now)   
    ...
    for i = 1 : obj.total_neurons
        ...
        size_post_syn_cell = size(post_syn_cell);
        length = max(size_post_syn_cell);   % # of post synaptic neuron
        
        for j = 1 : length
            switch post_syn_cell(j).ei_type
                case 'exc'
                    spike_time = find(obj.neuron_in_layer(i).spike_train); %spike train of pre synpatic neuron
                    
                    presyn_I = @() (1 / (post_syn_cell(j).taud - post_syn_cell(j).taur)...
                        * ( exp((time_now - spike_time) / taud) - ...
                        exp((time_now - spike_time) / taur) ) );
                    
                    recurrent_I = ( post_syn_cell(j).conn_strength / sqrt(keOutDegree) )...
                        * sum(presyn_I());
                    
                    %simulation in single neuron
                    post_syn_cell(j).postsyn_neuron.simulation(recurrent_I, time_now);
                ...
                case 'inh'
                    ...
                    recurrent_I = ( post_syn_cell(j).conn_strength / sqrt(kiOutDegree) )...
                        * sum(presyn_I());
