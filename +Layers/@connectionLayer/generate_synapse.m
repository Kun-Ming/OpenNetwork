...
%         i = pre_syn_id(j);
            ...
            for k = 1 : max(size(postsyn_IDs))
                ...
                if judgement < syn_type % slow syn
                    post_syn_arr{k} = Synapse.base_synapse( postsyn_neuron,...
                                                              layer1.neuron_in_layer(pre_syn_id(j)), ...
                                                              syn_param.slow.taud,...
                                                              syn_param.slow.taur,...
                                                              syn_param.slow.conn_strength.exc,...
                                                              'exc');
                    assert(post_syn_arr{k}.presyn_neuron  == layer1.neuron_in_layer(pre_syn_id(j)) &&...
                           post_syn_arr{k}.postsyn_neuron == postsyn_neuron, ...
                           'NOT SAME OBJECT');
                else
                    post_syn_arr{k} = Synapse.base_synapse( postsyn_neuron,...
                                                              layer1.neuron_in_layer(pre_syn_id(j)), ...
        ...
        obj.syn_in_layer{j} = post_syn_arr;
